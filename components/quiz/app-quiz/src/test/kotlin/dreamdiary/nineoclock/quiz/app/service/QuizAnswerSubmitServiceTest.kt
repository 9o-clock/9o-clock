...
class QuizAnswerSubmitServiceTest : QuizTestHelper() {
    ...
    fun submitQuizAnswer_authorize_submitter_or_throw() {
        BDDMockito.given(mockQuizActionAuthorizer.authorizeQuizSubmitAction(any())).willReturn(
            QuizAuthorizeQuizSubmitActionResult(false, "test")
        )
    ...
    fun submitQuizAnswer_find_quiz_or_throw() {
        BDDMockito.given(mockQuizRepository.findByPublicId(any())).willReturn(null)

        val exception = catchThrowableOfType({
            quizAnswerSubmitService.submitQuizAnswer(anQuizAnswerSubmitCommand())
        }, RuntimeException::class.java)

        verify(mockQuizRepository, times(1)).findByPublicId(capture(quizPublicIdCaptor))
        assertThat(exception).isNotNull()
    }
    ...
    fun submitQuizAnswer_save() {
        ...
        BDDMockito.given(mockQuizRepository.findByPublicId(any())).willReturn(givenQuiz)

        quizAnswerSubmitService.submitQuizAnswer(givenCommand)

        verify(mockQuizAnswerSubmitRepository, times(1)).save(capture(quizAnswerSubmitCaptor))
    }
}
